...
export function encounterFor(run){const seed=hash(run.seed,run.battleNumber*53+run.position.row),depth=run.battleNumber-1,base=depth<3?20+10*depth:25+12*depth,mini=run.selectedNode.kind==='miniboss',generated=makeEncounter(seed),enemyPieces=depth<3?2:Math.min(11,3+2*Math.floor(depth/2));return {...generated, enemy:depth<3?'striker':generated.enemy, enemyHp:Math.round(base*(mini?1.5:1)), enemyStrength:Math.floor(depth/3)+(mini?2:0),enemyPieces,enemyDepth:depth, miniboss:mini} }
